...
Api Key: eOWSopHkGDm8DzFzlI998QZdFf6E9GNE43PVsfp38J5fCfX00374hXTqIscnKsc0
...
- `FluentValidation` is used for strongly-typed validation on the incoming request. When the request is missing required fields or fails validation in anyway, a 400 error response is returned with status 'Rejected' alongside details of what was wrong with the request. No call is made to the Bank Simulator and no payment is persisted.
...
- If there are any exceptional errors, a 500 Internal Server Error response is returned, and the exception is logged. This situation can be simulated by turning off the Bank Simulator.
